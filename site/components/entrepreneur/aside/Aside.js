...
function normalizeEntrepreneurPath(pathname) {
  const raw = (pathname || "").split("?")[0];
  return raw.replace(/\/+$/, "") || "/";
}

function pathMatchesSegment(pathname, segment) {
  const p = normalizeEntrepreneurPath(pathname);
  if (segment === "" || segment === "overview") {
    return p === "/entrepreneur";
  }
  const prefix = `/entrepreneur/${segment}`;
  return p === prefix || p.startsWith(`${prefix}/`);
}

function getMostSpecificSubmenuMatch(submenu, pathname) {
  if (!Array.isArray(submenu) || submenu.length === 0) return null;
  const matches = submenu.filter((sub) => pathMatchesSegment(pathname, sub.link));
  if (!matches.length) return null;
  return matches.reduce((best, cur) => (cur.link.length > best.link.length ? cur : best));
}
...
const NAV_ITEMS = {
  ...
  orders: {
    ...
    hasSubmenu: false,
    submenu: [
      { link: "orders", label: "Manage Orders" },
  ...
  products: {
    ...
    hasSubmenu: true,
    submenu: [
      { link: "product", label: "Manage Products" },
  ...
  transaction: {
    ...
    submenu: [
      ...
      { link: "payout", label: "Payout Details" },
    ],
  },
  // reports: {
  //   icon: report_svg,
  ...
  settings: {
    ...
    hasSubmenu: true,
    submenu: [
      { link: "settings/shop", label: "Shop setup" },
      // { link: "settings/orders", label: "Order setup" },
      // { link: "settings/delivery", label: "Delivery setup" },
      // { link: "settings/inventory", label: "Inventory setup" },
      { link: "settings/payout", label: "Payout setup" },
  ...
  disputes: { icon: dispute_svg, label: "Disputes", hasSubmenu: false },
...
export default function Aside() {
  ...
  const entrepreneurShop = useSelector((s) => s.entrepreneur_shop?.shop);
  const activeShopId =
    entrepreneurShop?.id ??
    entrepreneurShop?.shop_id ??
    entrepreneurShop?.shopId ??
    getStoredEntrepreneurShopId();
  ...
  useEffect(() => {
    setAccordionState((prev) => {
      ...
      Object.entries(NAV_ITEMS).forEach(([key, item]) => {
        ...
        const match = getMostSpecificSubmenuMatch(item.submenu, pathname);
        if (match && !next[key]) {
          next[key] = true;
          changed = true;
        }
  ...
  //   let shops = getShopsByOwner(entrepreneurId);
  ...
  const handleNavigation = (link) => {
    if (link.split("/").splice(-1)[0] === "shop") {
      if (activeShopId != null) {
        window.open(`/entrepreneur/shop/${activeShopId}`, "_blank");
        return;
      }
    }
  ...
  const renderNavItem = (key, item) => {
    ...
    const simpleActive = pathMatchesSegment(pathname, key);
    ...
    return (
      <li key={key}>
        <div
          ...
          data-link={key}
          className={simpleActive ? "aside-nav-active" : undefined}
  ...
  const renderAccordionItem = (key, item) => {
    ...
    const activeSub = getMostSpecificSubmenuMatch(item.submenu, pathname);
    const parentActive = !!activeSub;
    ...
    return (
      <li key={key}>
        <div
          ...
          className={parentActive ? "aside-nav-active" : undefined}
          ...
          >
            ...
            {item.submenu.map((subItem, index) => (
              <div
                ...
                className={activeSub?.link === subItem.link ? "aside-submenu-active" : undefined}
                aria-current={activeSub?.link === subItem.link ? "page" : undefined}
              >
                <span></span>
  ...
  const renderResourceItem = (item, index) => {
    const resourceActive = pathMatchesSegment(pathname, item.link);
    return (
    <li key={index}>
      <div
        onClick={() => handleNavigation(item.link)}
        data-link={item.link}
        className={resourceActive ? "aside-nav-active" : undefined}
      ...
      <section className="links">
        ...
        <ul className="top-links">
          {Object.entries(NAV_ITEMS).map(([key, item]) =>
            renderNavItem(key === "overview" ? "" : key, item)
          )}
        </ul>
