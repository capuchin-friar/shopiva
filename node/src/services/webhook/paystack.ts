...
async function upsertNewOrder(payload: NewOrder){
    const pool = await db();
    const {
        customer_id, shop_id, amount_paid, tax, charges, total_paid, currency, fulfillment_status, escrow_status, payment_status, shipping_address, payment_reference, logistics_provider_id
    } = payload
    ...
    try{
        ...
        const { rows: [order] } = await pool.query(
            `INSERT INTO orders
            (
                customer_id, shop_id, amount_paid, tax, charges, total_paid, currency, fulfillment_status, escrow_status, payment_status, shipping_address, payment_reference, logistic_provider, created_at, updated_at
            ) VALUES(
